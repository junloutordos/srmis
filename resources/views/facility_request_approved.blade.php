...
<body>
  <div class="center">
    <div class="card" id="approvedCard">
      ...
      <h1>{{ $already ? 'Request Already Approved' : 'Facility Request Approved' }}</h1>
    ...
    (function(){
      ...
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'facilityRequestApproved', id: id, already: {{ $already ? 'true' : 'false' }} }, '*');
        }
      } catch (e){ }
      ...
      document.getElementById('returnBtn')?.addEventListener('click', function(){ try { if (window.opener && !window.opener.closed) window.opener.postMessage({ type: 'facilityRequestApproved', id: id }, '*'); } catch(e){} setTimeout(tryClose, 200); });
    })();
  </script>
</body>
</html>
