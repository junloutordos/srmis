...
<body>
  <div class="center">
    <div class="card" id="approvedCard">
      ...
      <h1>{{ $already ? 'Request Already Approved' : 'Messengerial Request Approved' }}</h1>
      <p class="meta">{{ $already ? 'This request was already approved.' : 'Thank you — the request has been approved successfully.' }}</p>
      <p class="meta">Request ID: {{ $messengerialRequest->id }}</p>
      <p><a class="btn" id="returnBtn" href="{{ url('/') }}">Return to Application</a></p>
    </div>
  </div>
  <script>
    (function(){
      const id = {{ $messengerialRequest->id }};
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'messengerialRequestApproved', id: id, already: {{ $already ? 'true' : 'false' }} }, '*');
        }
      } catch (e){ }
      ...
      document.getElementById('returnBtn')?.addEventListener('click', function(){ try { if (window.opener && !window.opener.closed) window.opener.postMessage({ type: 'messengerialRequestApproved', id: id }, '*'); } catch(e){} setTimeout(tryClose, 200); });
    })();
  </script>
</body>
</html>
