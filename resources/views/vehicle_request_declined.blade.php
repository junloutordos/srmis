...
<body>
  <div class="center">
    <div class="card">
      <h1>Vehicle Request Declined</h1>
  ...
  <script>
    (function(){
      ...
      // Notify opener (if present) so it can refresh
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'vehicleRequestDeclined', id: id, reason: reason }, '*');
        }
      } catch (e) { /* ignore */ }
      ...
      // Attempt to close after a short delay; fallback to redirect if close is blocked
      setTimeout(() => {
        tryClose();
